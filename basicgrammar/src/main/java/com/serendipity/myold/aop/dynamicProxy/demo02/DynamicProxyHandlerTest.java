...
public class DynamicProxyHandlerTest {
    ...
    private static <T> T getProxyInstance(Class<T> clazz) {
        DynamicProxyHandler<T> dynamicProxyHandler = new DynamicProxyHandler<>(clazz);
        return dynamicProxyHandler.getProxy();
    }

    public static void test(Class<?> clazz) throws InterruptedException {
        Object instance = getProxyInstance(clazz);
        switch (instance) {
            case Hello hello -> {
                hello.morning();
                hello.afternoon();
