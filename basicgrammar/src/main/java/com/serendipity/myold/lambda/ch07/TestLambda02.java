...
public class TestLambda02 {
    ...
    final List<Person> people = Arrays.asList(
            new Person("p1", "p1 message", 10),
            new Person("p1", "p2 message2222", 20),
            new Person("p2", "p2 message", 30),
            new Person("p3", "p3 message", 40),
            new Person("p4", "p4 message", 50)
    );
    ...
    public void test01() {
        ...
        final Double aDouble = list.stream().map((cost) -> cost + .12 * cost).reduce(Double::sum).get();
    ...
    public void test02() {
        ...
        final String collect = G7.stream().map(x -> x.toUpperCase()).collect(Collectors.joining(", "));
    ...
    @Test
    public void test05() {
        final long count = people.stream().filter(p -> p.getAge() > 30).count();
