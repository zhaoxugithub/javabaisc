...
public class TestReactive01 {
    ...
    @Test
    public void test02() {
        Flux.just(1).doOnRequest(System.out::println).doOnComplete(() -> System.out.println("over!")).doOnError(throwable -> System.out.println(throwable.getMessage())).subscribe();
        Flux.just("Ben", "Michael", "Mark").limitRequest(2) //  s.request(2);
                .doOnNext(s -> System.out.println("Hello " + s + "!"))
                // 在数据消费完之后执行
                .doOnComplete(() -> System.out.println("Completed"))
                // 在消费数据有异常的时候执行
                .doOnError(throwable -> System.out.println(throwable.getMessage())).subscribe();
    }


    @Test
    public void test03() {
        Flux.range(1, 10000).takeWhile(i -> i < 10).subscribe(System.out::println);
    }

    @Test
    public void test04() {
        Flux.just(1, 2).concatWith(Mono.error(new IllegalAccessError())).subscribe(System.out::println, System.err::println);
    }

    @Test
    public void test05() {
/*        Flux.just(1, 2).concatWith(Mono.error(new IllegalAccessError()))
                // 在数据有异常的时候返回
    ...
    public void test05_1() {
        // 有异常直接停止
/*
        Flux.range(1, 5).map(i -> i == 2 ? i / 0 : i).subscribe(System.out::println);
        Flux.range(1, 5).map(i -> i == 2 ? i / 0 : 1)
 ...
 */
        Flux.range(1, 5).map(i -> i == 2 ? i / 0 : 1).onErrorMap(err -> {
            System.out.println(err.getMessage());
            // 这里强制返回err
            return err;
        }).subscribe(System.out::println);
...
/*         Flux.range(1, 5)
        ...
        Flux.range(1, 5).flatMap(i -> {
            Mono.just(i).map(k -> k == 2 ? k / 0 : 1).onErrorResume(err -> {
    ...
    public void test06() {
        Flux.just(1, 2)
                // 在数据流的后面加上这个异常
                .concatWith(Mono.error(new IllegalStateException())).retry(1).subscribe(System.out::println);
    }
