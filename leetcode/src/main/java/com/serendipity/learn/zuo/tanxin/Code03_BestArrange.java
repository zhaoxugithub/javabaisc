...
public class Code03_BestArrange {
    ...
    public static int bestArrange3(Program[] programs) {
        ...
        boolean[] visited = new boolean[programs.length];
        int timeLine = 0;
    ...
    private static void process3(Program[] programs, int n, int index, boolean[] visited, int timeLine, TreeSet<Integer> result) {
        ...
        for (int i = 0; i < programs.length; i++) {
            if (programs[i].start > timeLine && !visited[i]) {
                visited[i] = true;
                process3(programs, n + 1, i, visited, programs[i].end, result);
                visited[i] = false;
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < timeTimes; i++) {
            ...
            int b1 = bestArrange1(programs);
            int b2 = bestArrange3(programs);
